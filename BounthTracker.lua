...
local FILE_NAME = "bounty_tracker_v4.txt"
...
-- ================= ANIMAÇÃO DO HOJE =================
local displayToday = today -- valor visual (animado)
...
local function animateToday(target)
	task.spawn(function()
		while math.floor(displayToday) ~= math.floor(target) do
			...
			-- velocidade dinâmica (quanto maior diferença, mais rápido)
			displayToday += diff * 0.15
			
			-- evitar travar em decimal
...
task.spawn(function()
	while true do
		...
		todayText.Text = "Hoje: "..format(today)
